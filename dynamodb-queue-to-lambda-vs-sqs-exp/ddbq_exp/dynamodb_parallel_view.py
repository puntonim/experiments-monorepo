...
logger.info("DYNAMODB PARELLEL: LOADING")


@logger.get_adapter().inject_lambda_context(log_event=True)
def lambda_handler(event: dict[str, Any], context: LambdaContext) -> None:
    """
    Handler for the Lambda function meant to be triggered concurrently by
     100 DynamoDB tasks of type DDBQ_EXP_PARALLEL.
    ...
    Example:
        To trigger this Lambda, write to the DynamoDB table arn:aws:dynamodb:eu-south-1:477353422995:table/ddbq-exp-task-prod
         a record like:
        {
          "PK": "DDBQ_EXP_PARALLEL#35Ks7iXolN7F096ufa4VHzW1Zdy",
          "SK": str(ksuid.KsuidMs()),
          "TaskId": "DDBQ_EXP_PARALLEL",
        ...
        {
            "PK": {"S": "DDBQ_EXP_PARALLEL#35Ks7iXolN7F096ufa4VHzW1Zdy"},
            "SK": {"S": "35Ks7iXolN7F096ufa4VHzW1Zdy"},
            "TaskId": {"S": "DDBQ_EXP_PARALLEL"},
            "ExpirationTs": {"N": "1762869932"},
            "SenderApp": {"S": "DDBQ_EXP_PRODUCER"},
            "Payload": {"M": {"text": {"S": "001"}}},
        }
    """
    logger.info("DYNAMODB PARELLEL: START")

    # Cast the event to the proper Lambda Powertools class.
    # dynamodb_event = DynamoDBStreamEvent(event)

    try:
        for task in dynamodb_task.DdbqDynamodbParallelTask.yield_from_event(event):
            key = f"root/parallel/{task.text}.txt"
